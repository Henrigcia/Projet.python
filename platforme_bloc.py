...
class PBloc:
    ...
    def add_platform(self, s : arcade.Sprite) -> None:                                          # Add another platform to the block
        ...
        self.d_b = min(self.d_b, self.boundary_right - s.center_x)                              # Recacluate the distance from the RIGHTMOST platform to the right boundary (update if the distance from the newlz added platform is less than the current distance)
        self.d_a = min(self.d_a, s.center_x - self.boundary_left)                               # Recacluate the distance from the LEFTMOST platform to the left boundary (update if the distance from the newlz added platform is less than the current distance)
